...
try:
    ...
    from assign_cases import assign_cases  # type: ignore
    from flag_quality import QualityResults, flag_all  # type: ignore
    from randomize import (  # type: ignore
        RandomizationConfig,
        RandomizationResult,
        Randomizer,
        TreatmentArm,
    )
    from power_calc import (  # type: ignore
        PowerAssumptions,
        calculate_mde,
        calculate_sample_size,
        generate_power_curve,
    ...
    from power_simulation import (  # type: ignore
        SimulationAssumptions,
        run_power_simulation,
        generate_simulation_power_curve,
        generate_simulation_stata_code,
    ...
    from backcheck import BackcheckConfig, sample_backchecks  # type: ignore
    from report import generate_weekly_report  # type: ignore
    ...
    if st.button("📄 Generate HTML Report", use_container_width=True, type="primary"):
        ...
        <head>
            ...
            <style>
                ...
                h1 {{ color: var(--primary-color); border-bottom: 3px solid #2fa6dc; padding-bottom: 10px; }}
                h2 {{ color: var(--primary-color); margin-top: 30px; }}
                .header {{ background: var(--secondary-background-color); padding: 20px; border-radius: 8px; margin-bottom: 30px; }}
                .section {{ margin: 20px 0; padding: 15px; background: var(--secondary-background-color); border-left: 4px solid #2fa6dc; }}
...
def render_quality_checks() -> None:
    ...
    if source == "Use project config":
        try:
            from monitor import load_submissions as mon_load_submissions
    ...
    with col2:
        st.markdown(
            ...
            <div style="text-align:center; padding: 3rem 0;">
                ...
                <p style="color: var(--text-color); font-size: 1.1rem;">This module is currently under development.<br>Check back soon!</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def show_temp_access_form(page_name: str):
    ...
    # Footer
    st.markdown("""
    <div style='text-align: center; margin-top: 50px; color: var(--text-color); font-size: 0.85rem;'>
...
def main() -> None:
    ...
    st.sidebar.markdown(
        """
        <div style="text-align: center; color: var(--text-color); font-size: 0.75rem; padding: 0.5rem 0;">
        ...
        <a href="mailto:aubreyjolex@gmail.com" style="color: var(--text-color); text-decoration: none;">📧 Email</a> | 
        <a href="https://github.com/ajolex/rct_field_flow" target="_blank" style="color: var(--text-color); text-decoration: none;">GitHub</a>
        </p>
        </div>
        """,
        unsafe_allow_html=True
...
def render_footer():
    ...
    with col2:
        st.markdown(
            """
            <div style="text-align: center; color: var(--text-color); font-size: 0.85rem; padding: 1rem 0;">
            <p style="margin: 0;">
